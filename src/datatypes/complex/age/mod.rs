...
//!
//! Confirmed invariants (source: the `Age` profile StructureDefinition JSON, error
//! severity throughout):
//! - `age-1`: `(code.exists() or value.empty()) and (system.empty() or system = %ucum)
//!   and (value.empty() or value.hasValue().not() or value > 0)` — "There SHALL be a
//!   code if there is a value and it SHALL be an expression of time. If system is
//!   present, it SHALL be UCUM. If value is present, it SHALL be positive." (`%ucum` is
//!   `http://unitsofmeasure.org`.) Fully machine-checkable from `value`/`system`/`code`
//!   alone and enforced here.
//! - `qty-3` (inherited from `Quantity`): `code.empty() or system.exists()` — enforced
//!   here too, since `age-1` alone doesn't cover a `code`-without-`value` case.
//! - `ele-1` (inherited from `Element`): at least one of
//!   `value`/`comparator`/`unit`/`system`/`code`/`extension` must be present.
//!
//! # Usage
...
/// The FHIR `Age` complex data type: a `Quantity` constrained to a positive,
/// UCUM time-unit value.
///
/// # Invariants
/// Any instance constructed via [`Age::new`] is guaranteed to satisfy `ele-1`, `qty-3`,
/// and `age-1` (see module docs).
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for AgeVisitor {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                f.write_str("a FHIR Age JSON object")
            }

            fn visit_map<A>(self, mut map: A) -> Result<Age, A::Error>
            where
                A: MapAccess<'de>,
            {
                let mut id = None;
                ...
                Age::new(value, comparator, unit, system, code, id, extension)
                    .map_err(DeError::custom)
            }
        }

        deserializer.deserialize_map(AgeVisitor)
    }
}

impl Age {
    /// Creates a new `Age`, validating the `ele-1`, `qty-3`, and `age-1` invariants.
    ///
    /// # Errors
    ...
    pub fn new(
        ...
        Self::validate_age1(&value, &system, &code)?;
        Ok(Self {
            id,
            extension,
            value,
    ...
    /// Validates the `age-1` invariant: a present `value` requires `code`, a present
    /// `system` must be UCUM, and a present `value` must be strictly positive.
    fn validate_age1(
        value: &Option<Primitive<Decimal>>,
        system: &Option<Primitive<Uri>>,
        code: &Option<Primitive<Code>>,
    ...
    /// Creates a new `Age` without validating the `ele-1`/`qty-3`/`age-1` invariants.
    ///
    /// # Warning
