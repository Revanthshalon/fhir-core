...
//! — the profile constrains `code`/`system` to a UCUM time-duration unit when present.
//! No additional fields beyond `Quantity`'s.
//!
//! # Status
...
/// The FHIR `Duration` complex data type: a `Quantity` constrained to a time-duration
/// unit.
