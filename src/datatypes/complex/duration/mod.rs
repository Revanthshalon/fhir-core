//! The FHIR `Duration` complex data type (a constrained `Quantity` profile).
//!
...
//!
//! Confirmed invariants (source: the `Duration` profile StructureDefinition JSON, error
//! severity throughout):
//! - `drt-1`: `code.exists() implies ((system = %ucum) and value.exists())` — "There
//!   SHALL be a code if there is a value and it SHALL be an expression of time. If
//!   system is present, it SHALL be UCUM." (`%ucum` is `http://unitsofmeasure.org`.)
//!   Note the direction of implication differs from `Age`/`Distance`/`Count`: it's
//!   `code` that drives the requirement here (`code` present -> `system` must be UCUM
//!   *and* `value` must be present), not `value` driving a `code` requirement. Fully
//!   machine-checkable from `value`/`system`/`code` alone and enforced here.
//! - `qty-3` (inherited from `Quantity`): `code.empty() or system.exists()` — implied
//!   by `drt-1` whenever `code` is present, but kept as a separate check for symmetry
//!   with the other profile types and in case `drt-1`'s scope narrows in a future spec
//!   revision.
//! - `ele-1` (inherited from `Element`).
//!
//! # Usage
//! Use [`Duration::new`] to construct a validated instance, or
//! [`Duration::new_unchecked`] when the fields are already known to satisfy the
//! invariants.
//!
//! # Ordering
//! `Duration` implements `PartialOrd` (not `Ord`) via the same comparator-aware,
...
/// The FHIR `Duration` complex data type: a `Quantity` constrained to a UCUM
/// time-duration unit.
///
/// # Invariants
/// Any instance constructed via [`Duration::new`] is guaranteed to satisfy `ele-1`,
/// `qty-3`, and `drt-1` (see module docs).
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for DurationVisitor {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                f.write_str("a FHIR Duration JSON object")
            }
                ...
                A: MapAccess<'de>,
            {
                ...
                Duration::new(value, comparator, unit, system, code, id, extension)
...
impl Duration {
    /// Creates a new `Duration`, validating the `ele-1`, `qty-3`, and `drt-1`
    /// invariants.
    ///
    /// # Errors
    ...
    pub fn new(
        ...
        Self::validate_drt1(&value, &system, &code)?;
        Ok(Self {
            id,
            extension,
            value,
    ...
    /// Validates the `drt-1` invariant: a present `code` requires `system` to be UCUM
    /// and `value` to be present.
    fn validate_drt1(
        value: &Option<Primitive<Decimal>>,
        system: &Option<Primitive<Uri>>,
        code: &Option<Primitive<Code>>,
    ...
    /// Creates a new `Duration` without validating the `ele-1`/`qty-3`/`drt-1`
    /// invariants.
    ///
    /// # Warning
