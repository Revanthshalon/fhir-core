...
//!
//! Confirmed invariants (source: the `Distance` profile StructureDefinition JSON, error
//! severity throughout):
//! - `dis-1`: `(code.exists() or value.empty()) and (system.empty() or system = %ucum)`
//!   — "There SHALL be a code if there is a value and it SHALL be an expression of
//!   length. If system is present, it SHALL be UCUM." (`%ucum` is
...
//!
//! # Usage
//! Use [`Distance::new`] to construct a validated instance, or
//! [`Distance::new_unchecked`] when the fields are already known to satisfy the
//! invariants.
//!
//! # Ordering
//! `Distance` implements `PartialOrd` (not `Ord`) via the same comparator-aware,
...
/// The FHIR `Distance` complex data type: a `Quantity` constrained to a UCUM length
/// unit.
///
/// # Invariants
/// Any instance constructed via [`Distance::new`] is guaranteed to satisfy `ele-1`,
/// `qty-3`, and `dis-1` (see module docs).
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for DistanceVisitor {
            ...
            fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                f.write_str("a FHIR Distance JSON object")
            }

            fn visit_map<A>(self, mut map: A) -> Result<Distance, A::Error>
            where
                A: MapAccess<'de>,
            {
                let mut id = None;
                ...
                Distance::new(value, comparator, unit, system, code, id, extension)
                    .map_err(DeError::custom)
            }
        }

        deserializer.deserialize_map(DistanceVisitor)
    }
}

impl Distance {
    /// Creates a new `Distance`, validating the `ele-1`, `qty-3`, and `dis-1`
    /// invariants.
    ///
    /// # Errors
    ...
    pub fn new(
        ...
        Self::validate_dis1(&value, &system, &code)?;
        Ok(Self {
            id,
            extension,
            value,
    ...
    /// Validates the `dis-1` invariant: a present `value` requires `code`, and a
    /// present `system` must be UCUM.
    fn validate_dis1(
        value: &Option<Primitive<Decimal>>,
        system: &Option<Primitive<Uri>>,
        code: &Option<Primitive<Code>>,
        ...
        let description = "There SHALL be a code if there is a value and it SHALL be an \
            expression of length. If system is present, it SHALL be UCUM."
            .to_owned();
    ...
    /// Creates a new `Distance` without validating the `ele-1`/`qty-3`/`dis-1`
    /// invariants.
    ///
    /// # Warning
